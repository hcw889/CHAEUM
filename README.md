...
**매칭 요청이 들어올 때마다 희망 지역을 기준으로 두 API를 새로 조회한다**
(`LiveSanggaProvider`). 서비스키만 있으면 사전 수집 없이도 뜬다. provider 선택 순서:

| 조건 | provider | 후보 |
| --- | --- | --- |
| 두 API 서비스키 있음 | `LiveSanggaProvider` | 요청 시점 라이브 검색 |
| 키 없고 `app/data/real/` 캐시만 있음 | `RealSanggaProvider` | 마지막 수집분 |
| 둘 다 없음 | `MockDataProvider` | 시연용 목업 |

`CHAEUM_LIVE_SEARCH=0`을 주면 라이브 검색을 끄고 디스크 캐시 방식으로 돌아간다.
...
3. 서버를 띄우면 바로 라이브 검색이 동작한다. 아래 수집 스크립트는 **선택**이며,
   미리 돌려 두면 그 지역의 첫 검색이 빨라진다(대장 캐시를 같이 채운다) — 그리고
   라이브 조회가 실패했을 때의 폴백이 된다. venv를 반드시 쓴다(시스템 Python에는
   의존성이 없다).
...
4. 결과 화면 상단에 "실데이터 기반 공실 추정" 배너가 뜬다. `/api/match`의
   `source_note`가 어느 지점 반경을 방금 조회했는지 알려 준다.
...
건축물대장은 건물당 2회 호출이라 그냥 던지면 검색 한 번이 68초까지 늘어난다
(개발계정 429 + 지수 백오프, 실측). 두 겹으로 줄인다:

| 캐시 | 대상 | 수명 | 이유 |
| --- | --- | --- | --- |
| TTL 캐시 (메모리) | 검색 결과 전체 | 15분 | 같은 지역 재검색을 즉시 응답 |
| 디스크 캐시 (sqlite) | 건축물대장 응답 | 90일 | 사용승인일·층별 용도는 준공 후 거의 안 바뀐다 |

공실 판정의 실제 신호인 **상가정보(영업 점포 목록)는 매 검색마다 새로 부른다** —
대장을 캐시해도 공실 판정의 신선도는 떨어지지 않는다. 실측 소요: 첫 검색 약 20초,
대장 캐시가 warm하면 약 7초, TTL 캐시 적중이면 즉시.

디스크 캐시는 `backend/app/data/cache/register.sqlite3`이며 git에 올리지 않는다.
지우면 다음 검색이 다시 채운다.
