...
건물 분석(`tests/analysis.spec.ts`), 매칭 지도·상세 팝업과 유동인구 검사는 고정된 API 응답을 사용하므로 외부 API 키 없이 실행할 수 있습니다. 분석 페이지는 일곱 섹션의 연속 표시, 고정 메뉴와 스크롤 위치, 조회 공유·입력 상태 유지, 부분 실패 후 재시도, 기존 주소의 섹션 연결과 리포트 전용 인쇄를 검사합니다. 지도 순위 마커·선택 매물의 확대 지도·키보드 포커스·모바일 배치·이전 세션의 좌표 조회도 검사합니다. 실제 SK 데이터 연동과 추천 품질은 별도 검증 대상입니다. 백엔드 pytest 실행은 [프로젝트 README의 테스트 안내](../README.md#테스트)를 참조합니다.
