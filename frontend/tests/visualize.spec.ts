...
test("직접 진입과 새로고침으로 유동인구를 조회하며 이미지 생성 기능은 없다", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "유동인구", exact: true })).toHaveAttribute("href", "/visualize/b1");
  await expect(page.getByRole("link", { name: "리포트 보기 →" })).toHaveAttribute("href", "/report/b1");
...
test("매물 선택기를 바꾸면 새 ID를 조회하고 이전 선택을 초기화한다", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "리포트 보기 →" })).toHaveAttribute("href", "/report/b2");
});
